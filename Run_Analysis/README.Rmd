...
#README Help File With Code Snippets

Reading the data in and extracting the zip file
```{r}
if(file.exists("./data")) {
        setwd("./data")
} else {
...
Assigning tables to appropriate files and merging data
```{r}
library(plyr)
...
creating a list with the above merged data tables
```{r}
list(x=reading_measures, y=activity_measures, subject=subject_measures)
```

creating a data frame for mean and standard deviation
```{r}
...
creating a dataframe for the activity names
```{r}
...
binding the three merged data sets
```{r}
